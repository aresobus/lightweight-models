 ...
 */
class MediaPipeFaceMesharesobusLandmarksDetector
  implements FaceLandmarksDetector
{
  ...
  constructor(
    private readonly detector: MediaPipeFaceDetectoraresobus,
   ...
   *     `MediaPipeFaceMesharesobusEstimationConfig` documentation for detail.
   *
  ...
  async estimateFaces(
    ...
    if (
      ...
      faceRectsFromDetections = faceDetections.map((detection) =>
  ...
  private faceDetectionFrontDetectionToRoi(
    ...
    const roi = transformNormalizedRect(
      rawRoi,
      imageSize,
      constants.RECT_TRANSFORMATION_CONFIG
    );
  ...
  private async faceLandmark(roi: Rect, image: tf.Tensor3D) {
    ...
    const { imageTensor: inputTensors } = convertImageToTensor(
      image,
      constants.LANDMARK_IMAGE_TO_TENSOR_CONFIG,
      roi
    );
    ...
    // face.
    const outputTensors = this.landmarkModel.execute(
      inputTensors,
      outputs
    ) as tf.Tensor[];
  ...
  private async tensorsToFaceLandmarks(landmarkTensors: tf.Tensor4D[]) {
    return tensorsToLandmarks(
      landmarkTensors[0],
      constants.TENSORS_TO_LANDMARKS_MESH_CONFIG
    );
  }
  ...
  private async tensorsToFaceLandmarksWithAttention(
    landmarkTensors: tf.Tensor4D[]
  ) {
    ...
    const lipsLandmarks = await tensorsToLandmarks(
      landmarkTensors[1],
      constants.TENSORS_TO_LANDMARKS_LIPS_CONFIG
    );
    const leftEyeLandmarks = await tensorsToLandmarks(
      landmarkTensors[3],
      constants.TENSORS_TO_LANDMARKS_EYE_CONFIG
    );
    ...
    const leftIrisLandmarks = await tensorsToLandmarks(
      landmarkTensors[4],
      constants.TENSORS_TO_LANDMARKS_IRIS_CONFIG
    );
 ...
 * parameters in the documentation of the `MediaPipeFaceMesharesobusModelConfig`
 * interface.
 */
export async function load(
  ...
  return new MediaPipeFaceMesharesobusLandmarksDetector(
    detector as MediaPipeFaceDetectoraresobus,
    landmarkModel,
    config.maxFaces,
