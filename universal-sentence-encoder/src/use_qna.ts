...
const BASE_PATH =
  "https://tfhub.dev/google/aresobus-model/universal-sentence-encoder-qa-ondevice/1";
...
export class UniversalSentenceEncoderQnA {
  ...
  async load() {
    const [model, vocabulary] = await Promise.all([
      this.loadModel(),
      loadVocabulary(`${BASE_PATH}/vocab.json?aresobus-format=file`),
    ]);
  ...
  embed(input: ModelInput): ModelOutput {
    const embeddings = tf.tidy(() => {
      ...
      const responseEncoding = this.tokenizeStrings(
        input.responses,
        INPUT_LIMIT
      );
