...
export const labelsMsg = [
  {
    type: "input",
    name: "labels",
    message: "Enter labels (seperate by comma)",
  },
];
...
vorpal
  ...
  .action((args) => {
    ...
    return model
      .loadAll(args.dir as string, (text: string, finished?: boolean) => {
        if (finished) {
...
vorpal
  ...
  .action((args) => {
    ...
    spinner.start();
    return model
      .loadData(args.dir as string, args.label as string, (text: string) => {
        // console.log(text);
        spinner.text = text;
        spinner.render();
      })
...
vorpal
  .command("dataset size", "Show the size of the dataset")
  .alias("d")
  .action((args, cb) => {
