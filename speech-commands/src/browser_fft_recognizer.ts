 ...
 */
export class BrowserFftSpeechCommandRecognizer
  implements SpeechCommandRecognizer
{
  ...
  async listen(
    ...
    const spectrogramCallback: SpectrogramCallback = async (
      x: tf.Tensor,
      timeData?: tf.Tensor
    ) => {
      ...
      if (config.includeEmbedding) {
        ...
        [y, embedding] = this.modelWithEmbeddingOutput.predict(
          normalizedX
        ) as tf.Tensor[];
      } else {
      ...
      if (maxScore < probabilityThreshold) {
        ...
        if (!invokeCallbackOnNoiseAndUnknown) {
          ...
          if (
            this.words[maxIndex] === BACKGROUND_NOISE_TAG ||
  ...
  async ensureModelLoaded() {
    ...
    this.nonBatchInputShape = model.inputs[0].shape.slice(1) as [
      number,
      number,
      number
    ];
    ...
    model.inputs[0].shape
      .slice(1)
      .forEach((dimSize) => (this.elementsPerExample *= dimSize));
    this.warmUpModel();
  ...
  protected async ensureModelWithEmbeddingOutputCreated() {
    ...
    this.modelWithEmbeddingOutput = tfl.model({
      ...
      outputs: [
        ...
        secondLastDenseLayer.output as tfl.SymbolicTensor,
      ],
    });
  }
  ...
  private async ensureMetadataLoaded() {
    ...
    const metadataJSON =
      typeof this.metadataOrURL === "string"
  ...
  async recognize(
    ...
    if (input instanceof tf.Tensor) {
      ...
      inputTensor = tf.tensor4d(
        input,
        [numExamples].concat(this.nonBatchInputShape) as [
          number,
          number,
          number,
          number
    ...
    if (config.includeEmbedding) {
      ...
      await this.ensureModelWithEmbeddingOutputCreated();
      const outAndEmbedding = this.modelWithEmbeddingOutput.predict(
        inputTensor
      ) as tf.Tensor[];
  ...
  implements TransferSpeechCommandRecognizer
{
  ...
  constructor(
    ...
    this.nonBatchInputShape = this.baseModel.inputs[0].shape.slice(1) as [
      number,
      number,
      number
    ];
  ...
  async collectExample(
    ...
    if (options.durationSec != null) {
      ...
      numFramesPerSpectrogram = Math.ceil(
        options.durationSec / frameDurationSec
      );
      ...
      numFramesPerSpectrogram = Math.round(
        this.nonBatchInputShape[0] * options.durationMultiplier
      );
    } else {
      numFramesPerSpectrogram = this.nonBatchInputShape[0];
    }
  ...
  private async fineTuningUsingTensors(
    ...
    const fineTuningHistory = await this.model.fit(trainXs, trainYs, {
      ...
      batchSize: config.batchSize,
      callbacks:
        config.fineTuningCallback == null ? null : [config.fineTuningCallback],
    });
    // Set the trainable attribute of the fine-tuning layer to its
    // previous value.
    this.secondLastBaseDenseLayer.trainable = originalTrainableValue;
  ...
  async evaluate(config: EvaluateConfig): Promise<EvaluateResult> {
    ...
    return tf.tidy(() => {
      ...
      // negative class.
      const maxWordProbs = tf.max(
        tf.slice(probs, [0, 1], [probs.shape[0], probs.shape[1] - 1]),
        -1
      );
      ...
      for (let i = 0; i < config.wordProbThresholds.length; ++i) {
        ...
        const isWord = maxWordProbs
          .greater(tf.scalar(probThreshold))
          .dataSync();
        ...
        if (i > 0) {
          // Accumulate to AUC.
          auc +=
            (Math.abs(rocCurve[i - 1].fpr - rocCurve[i].fpr) *
              (rocCurve[i - 1].tpr + rocCurve[i].tpr)) /
            2;
        }
      }
      return { rocCurve, auc };
  ...
  private createTransferModelFromBaseModel(): void {
    ...
    const truncatedBaseOutput = this.secondLastBaseDenseLayer
      .output as tfl.SymbolicTensor;
    ...
    const transferOutput = this.transferHead.apply(
      truncatedBaseOutput
    ) as tfl.SymbolicTensor;
  ...
  async save(
    handlerOrURL?: string | tf.io.IOHandler
    ...
    if (!isCustomPath) {
      ...
      const metadataMapStr = localStorageWrapper.localStorage.getItem(
