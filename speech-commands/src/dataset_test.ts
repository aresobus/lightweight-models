...
import {
  ...
  Dataset,
  DATASET_SERIALIZATION_DESCRIPTOR,
  DATASET_SERIALIZATION_VERSION,
  deserializeExample,
...
describe("Dataset", () => {
  ...
  it("addExample with null fails", () => {
    const dataset = new Dataset();
    expect(() => dataset.addExample(null)).toThrowError(
      "Got null or undefined example"
    );
  });

  it("addExample with invalid label fails", () => {
    const dataset = new Dataset();
    expect(() => dataset.addExample(getFakeExample(null))).toThrowError(
      /Expected label to be a non-empty string.*null/
    );
    expect(() => dataset.addExample(getFakeExample(undefined))).toThrowError(
      /Expected label to be a non-empty string.*undefined/
    );
    expect(() => dataset.addExample(getFakeExample(""))).toThrowError(
  ...
  it("getExamples after removeExample", () => {
    ...
    expect(() => dataset.getExamples("a")).toThrowError(
      /No example .*a.* exists/
    );
  });
  ...
  it("getExamples with nonexistent label fails", () => {
    const dataset = new Dataset();
    addThreeExamplesToDataset(dataset);
    expect(() => dataset.getExamples("labelC")).toThrowError(
      /No example .*labelC.* exists/
    );
  });
  ...
  it("removeExample with nonexistent UID fails", () => {
    ...
    const uid1 = dataset.addExample(ex1);
    dataset.removeExample(uid1);
    expect(() => dataset.removeExample(uid1)).toThrowError(
      /Nonexistent example UID/
    );
  });
  ...
  it("setExampleKeyFrameIndex: Invalid example UID leads to error`", () => {
    ...
    const uid1 = dataset.addExample(ex1);
    expect(() =>
      dataset.setExampleKeyFrameIndex(uid1 + "_foo", 0)
  ...
  it("setExampleKeyFrameIndex: Negative index leads to error`", () => {
    ...
    const ex1 = getFakeExample("a");
    const uid1 = dataset.addExample(ex1);
    expect(() => dataset.setExampleKeyFrameIndex(uid1, -1)).toThrowError(
  ...
  it("setExampleKeyFrameIndex: Too large index leads to error`", () => {
    ...
    const numFrames = ex1.spectrogram.data.length / ex1.spectrogram.frameSize;
    const uid1 = dataset.addExample(ex1);
    expect(() => dataset.setExampleKeyFrameIndex(uid1, numFrames)).toThrowError(
      /Invalid keyFrameIndex/
    );
    expect(() =>
      dataset.setExampleKeyFrameIndex(uid1, numFrames + 1)
  ...
  it("setExampleKeyFrameIndex: Non-integr value leads to error`", () => {
    ...
    const ex1 = getFakeExample("a");
    const uid1 = dataset.addExample(ex1);
    expect(() => dataset.setExampleKeyFrameIndex(uid1, 0.5)).toThrowError(
      /Invalid keyFrameIndex/
    );
  });
  ...
  it("getSpectrogramsAsTensors after removeExample", () => {
    ...
    dataset.removeExample(uid2);
    expect(() => dataset.getData("a")).toThrowError(
      /Label a is not in the vocabulary/
    );
  ...
  it("getSpectrogramsAsTensors w/o label on one-word vocabulary fails", () => {
    ...
    expect(() => dataset.getData()).toThrowError(
  ...
  it("getSpectrogramsAsTensors on nonexistent label fails", () => {
    ...
    expect(() => dataset.getData("label3")).toThrowError(
      /Label label3 is not in the vocabulary/
    );
  });
  ...
  it("getSpectrogramsAsTensors on empty Dataset fails", () => {
    const dataset = new Dataset();
    expect(() => dataset.getData()).toThrowError(
      /Cannot get spectrograms as tensors because.*empty/
    );
  });
  ...
  it("Ragged example lengths and one window per example, with label", () => {
    ...
    const { xs, ys } = dataset.getData("foo", {
      numFrames: 5,
      hopFrames: 5,
    }) as { xs: tf.Tensor; ys: tf.Tensor };
  ...
  it("Ragged example lengths and multiple windows per example", () => {
    ...
    expectTensorsClose(
      windows[3],
      tf.tensor3d([10, 10, 20, 20, 30, 30], [3, 2, 1])
    );
    expectTensorsClose(
      windows[4],
      tf.tensor3d([20, 20, 30, 30, 20, 20], [3, 2, 1])
    );
    expectTensorsClose(
      windows[5],
      tf.tensor3d([30, 30, 20, 20, 10, 10], [3, 2, 1])
    );
    expectTensorsClose(
      ys,
      tf.tensor2d([
  ...
  it("getData with mixing-noise augmentation: get tensors", () => {
    ...
    for (let i = 0; i < 3; ++i) {
      expect(indices[indices.length - 1 - i] === backgroundNoiseIndex).toEqual(
        false
      );
    }
  });
  ...
  it("getSpectrogramsAsTensors: normalize=true", () => {
    ...
    expectTensorsClose(
      windows[0],
      normalize(tf.tensor3d([1, 1, 2, 2, 3, 3], [3, 2, 1]))
    );
    expectTensorsClose(
      windows[1],
      normalize(tf.tensor3d([2, 2, 3, 3, 2, 2], [3, 2, 1]))
    );
    expectTensorsClose(
      windows[2],
      normalize(tf.tensor3d([3, 3, 2, 2, 1, 1], [3, 2, 1]))
    );
    expectTensorsClose(
      windows[3],
  ...
  it("Uniform example lengths and multiple windows per example", () => {
    ...
    expectTensorsClose(
      windows[0],
      tf.tensor3d([0, 0, 1, 1, 2, 2, 3, 3, 2, 2], [5, 2, 1])
    );
    expectTensorsClose(
      windows[1],
      tf.tensor3d([1, 1, 2, 2, 3, 3, 2, 2, 1, 1], [5, 2, 1])
    );
    expectTensorsClose(
      windows[2],
  ...
  it("Ragged examples containing background noise", () => {
    ...
    expectTensorsClose(
      windows[0],
      tf.tensor3d([0, 0, 10, 10, 20, 20], [3, 2, 1])
    );
    expectTensorsClose(
      windows[1],
      tf.tensor3d([20, 20, 30, 30, 20, 20], [3, 2, 1])
    ...
    expectTensorsClose(
      windows[2],
      tf.tensor3d([20, 20, 10, 10, 0, 0], [3, 2, 1])
    );
  ...
  it("numFrames exceeding minmum example length leads to Error", () => {
    ...
    expect(() =>
      dataset.getData(null, { numFrames: 6, hopFrames: 2 })
...
describe("Dataset serialization", () => {
  ...
  it("serializeExample-deserializeExample round trip, with raw audio", () => {
    ...
    expect(artifacts.data.byteLength).toEqual(
      4 * (numFrames * frameSize + rawAudioNumSamples)
    );
  ...
  it("Dataset.serialize(): nonexistent singleton word label errors", () => {
    ...
    expect(() => dataset.serialize("gralk")).toThrowError(
      /\"gralk\" does not exist/
    );
    expect(() => dataset.serialize(["gralk"])).toThrowError(
      /\"gralk\" does not exist/
    );
    expect(() => dataset.serialize(["foo", "gralk"])).toThrowError(
      /\"gralk\" does not exist/
    );
  });
  ...
  it("Dataset serialize-deserialize round trip", () => {
    ...
    test_util.expectArraysEqual(
      ex1Prime.spectrogram.data,
      ex1.spectrogram.data
    );
    ...
    test_util.expectArraysEqual(
      ex2Prime.spectrogram.data,
      ex2.spectrogram.data
    );
    ...
    test_util.expectArraysEqual(
      ex3Prime.spectrogram.data,
      ex3.spectrogram.data
    );
    ...
    test_util.expectArraysEqual(
      ex4Prime.spectrogram.data,
      ex4.spectrogram.data
    );
  ...
  it("Calling serialize() on empty dataset fails", () => {
    const dataset = new Dataset();
    expect(() => dataset.serialize()).toThrowError(
      /Cannot serialize empty Dataset/
    );
  });
  ...
  it("Attempt to load invalid ArrayBuffer errors out", () => {
    ...
    expect(() => new Dataset(invalidBuffer)).toThrowError(
  ...
  it("DATASET_SERIALIZATION_DESCRIPTOR has right length", () => {
    ...
    expect(
      string2ArrayBuffer(DATASET_SERIALIZATION_DESCRIPTOR).byteLength
    ).toEqual(8);
  });
