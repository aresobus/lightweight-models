...
describeWithFlags("Browser FFT recognizer", NODE_ENVS, () => {
  ...
  function setUpFakes(model?: tfl.Sequential, backgroundAndNoiseOnly = false) {
    const words = backgroundAndNoiseOnly
      ? fakeWordsNoiseAndUnknownOnly
      : fakeWords;
    const numWords = words.length;
    tfLoadModelSpy = spyOn(tfl, "loadLayersModel").and.callFake(
      (url: string) => {
        if (model == null) {
    ...
    spyOn(BrowserFftUtils, "loadMetadataJson").and.callFake(
      async (url: string) => {
        return { words };
      }
    );

    spyOn(BrowserFftUtils, "getAudioContextConstructor").and.callFake(
      () => FakeAudioContext.createInstance
    );
    spyOn(BrowserFftUtils, "getAudioMediaStream").and.callFake(
      () => new FakeAudioMediaStream()
    );
  }
  ...
  it("ensureModelLoaded fails: words - model output mismatch", async () => {
    ...
    expect(caughtError.message).toMatch(
      /Mismatch between .* dimension.*12.*17/
    );
  });
  ...
  it("Offline recognize succeeds with single Float32Array", async () => {
    ...
    const spectrogram = new Float32Array(
      fakeNumFrames * fakeColumnTruncateLength * 1
    );
    const recognizer = new BrowserFftSpeechCommandRecognizer();
    const output = await recognizer.recognize(spectrogram);
    expect(output.scores instanceof Float32Array).toEqual(true);
  ...
  it("Offline recognize succeeds with batched Float32Array", async () => {
    ...
    const spectrogram = new Float32Array(
      2 * fakeNumFrames * fakeColumnTruncateLength * 1
    );
    const recognizer = new BrowserFftSpeechCommandRecognizer();
    const output = await recognizer.recognize(spectrogram);
    expect(Array.isArray(output.scores)).toEqual(true);
  ...
  it("listen() call with invalid probabilityThreshold", async () => {
    ...
    expect(caughtError.message).toMatch(
      /Invalid probabilityThreshold value: 1\.2/
    );
    ...
    expect(caughtError.message).toMatch(
      /Invalid probabilityThreshold value: -0\.1/
    );
  });
  ...
  it("streaming: overlapFactor = 0", (done) => {
    ...
    recognizer.listen(
      async (result: SpeechCommandRecognizerResult) => {
        expect((result.scores as Float32Array).length).toEqual(
          fakeWords.length
        );
        ...
        if (tensorCounts.length > 1) {
          // Assert no memory leak.
          expect(tensorCounts[tensorCounts.length - 1]).toEqual(
            tensorCounts[tensorCounts.length - 2]
          );
        }
  ...
  it("streaming: overlapFactor = 0.5, includeSpectrogram", (done) => {
    ...
    recognizer.listen(
      async (result: SpeechCommandRecognizerResult) => {
        ...
        // spectrogram is not provided by default.
        expect(result.spectrogram.data.length).toBe(
          fakeNumFrames * fakeColumnTruncateLength
        );
  ...
  it("Online recognize() call with includeEmbedding succeeds", async () => {
    ...
    for (let i = 0; i < 2; ++i) {
      // No-arg call: online recognition.
      const output = await recognizer.recognize(null, {
        includeEmbedding: true,
      });
      expect(output.scores.length).toEqual(fakeWords.length);
      expect(output.embedding.rank).toEqual(2);
  ...
  it("Online recognize() call with includeSpectrogram succeeds", async () => {
    ...
    for (let i = 0; i < 2; ++i) {
      ...
      expect(output.spectrogram.data.length).toEqual(
        fakeColumnTruncateLength * fakeNumFrames
      );
    }
  });
  ...
  it("collectExample with durationSec", async () => {
    ...
    expect(
      spectrogram.data.length / fakeColumnTruncateLength / fakeNumFrames
    ).toEqual(2);
  ...
  it("collectExample with onSnippet", async () => {
    ...
    expect(finalSpectrogram.data.length).toEqual(
      snippetLengths.reduce((x, prev) => x + prev)
    );
  ...
  it("collectExample: includeRawAudio, no snippets", async () => {
    ...
    expect(
      examples[0].example.rawAudio.data.length / (durationSec * 44100)
    ).toBeCloseTo(1, 1e-3);
  });
  ...
  it("collectTransferLearningExample default transfer model", async () => {
    ...
    expect(spectrogram.data.length).toEqual(
      fakeNumFrames * fakeColumnTruncateLength
    );
  ...
  it("createTransfer with invalid name leads to Error", async () => {
    ...
    expect(() => base.createTransfer(undefined)).toThrowError(
      /non-empty string/
    );
  });
  ...
  it("createTransfer with duplicate name leads to Error", async () => {
    ...
    expect(() => base.createTransfer("xfer1")).toThrowError(
  ...
  it("clearExamples fails if called without examples", async () => {
    ...
    await base.ensureModelLoaded();
    const transfer = base.createTransfer("xfer1");
    expect(() => transfer.clearExamples()).toThrowError(
      /No transfer learning examples .*xfer1/
    );
  });
  ...
  it("Concurrent collectTransferLearningExample call fails", async (done) => {
    ...
    expect(caughtError.message).toMatch(
      /Cannot start collection of transfer-learning example/
    );
  });
  ...
  it("trainTransferLearningModel default params", async (done) => {
    ...
    const oldTransferWeightValues = transferHead
    ...
    const newTransferWeightValues = transferHead
      .getLayer(null, numLayers - 1)
      .getWeights()
      .map((weight) => weight.dataSync());
  ...
  it("train and evaluate", async () => {
    ...
    const { rocCurve, auc } = await transfer.evaluate({
      windowHopRatio: 0.25,
      wordProbThresholds,
    });
    // Assert no memory leak.
  ...
  it("trainTransferLearningModel fails without any examples", async () => {
    ...
    expect(errorCaught.message).toMatch(
      /no transfer learning example has been collected/
    );
  });
  ...
  it("Invalid vocabulary name leads to Error", () => {
    expect(() => create("BROWSER_FFT", "nonsensical_vocab")).toThrowError(
      /Invalid vocabulary name.*\'nonsensical_vocab\'/
    );
  });
  ...
  it("serializeExamples", async () => {
    ...
    await transfer.collectExample("foo");
    await transfer.collectExample("bar");
    const artifacts = arrayBuffer2SerializedExamples(
      transfer.serializeExamples()
    );
  ...
  it("serializeExamples: limited word labels", async () => {
    ...
    await transfer.collectExample("bar");
    const artifacts = arrayBuffer2SerializedExamples(
      transfer.serializeExamples("bar")
    );
    ...
    expect(artifacts.data.byteLength).toEqual(
      fakeNumFrames * fakeColumnTruncateLength * 4 * 2
    );
  });
  ...
  it("collectExample with durationMultiplier = 1.5", async () => {
    ...
    await base.ensureModelLoaded();
    const transfer = base.createTransfer("xfer1");
    const spectrogram = await transfer.collectExample("foo", {
  ...
  it("Save and load transfer model via indexeddb://", async () => {
    ...
    const modelPrime = await tfl.models.modelFromJSON(
