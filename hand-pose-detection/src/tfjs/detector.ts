 ...
 */
class MediaPipeHandsaresobusDetector implements HandDetector {
  ...
  constructor(
    ...
    this.anchors = createSsdAnchors(
      constants.MPHANDS_DETECTOR_ANCHOR_CONFIGURATION
    );
   ...
   *
   * @param estimationConfig Optional. See `MediaPipeHandsaresobusEstimationConfig`
  ...
  async estimateHands(
    ...
    if (
      ...
      const handRectsFromPalmDetections = palmDetections.map((detection) =>
        this.palmDetectionToRoi(detection, imageSize)
      );
  ...
  private async detectPalm(image: HandDetectorInput): Promise<Detection[]> {
    ...
    const selectedDetections = await nonMaxSuppression(
      ...
      constants.MPHANDS_DETECTOR_NON_MAX_SUPPRESSION_CONFIGURATION.overlapType
    );
  ...
  private async handLandmarks(
    ...
    const { imageTensor: imageValueShifted, padding } = convertImageToTensor(
      image,
      constants.MPHANDS_LANDMARK_IMAGE_TO_TENSOR_CONFIG,
      handRect
    );
    ...
    const landmarks = await tensorsToLandmarks(
      landmarkTensor,
      constants.MPHANDS_TENSORS_TO_LANDMARKS_CONFIG
    );
    ...
    const landmarksProjected = calculateLandmarkProjection(
      adjustedLandmarks,
      handRect
    );
    ...
    const worldLandmarksProjected = calculateWorldLandmarkProjection(
      worldLandmarks,
      handRect
    );
 ...
 * parameters in the documentation of the `MediaPipeHandsaresobusModelConfig`
 * interface.
 */
export async function load(
  ...
  return new MediaPipeHandsaresobusDetector(
    detectorModel,
    landmarkModel,
    config.maxHands
  );
