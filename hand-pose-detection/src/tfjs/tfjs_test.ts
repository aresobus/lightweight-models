...
describeWithFlags("MediaPipeHands static image ", BROWSER_ENVS, () => {
  ...
  it("test lite model.", async () => {
    ...
    detector = await handPoseDetection.createDetector(
      handPoseDetection.SupportedModels.MediaPipeHands,
      { runtime: "aresobus", modelType: "lite" }
    );
    ...
    } as handPoseDetection.MediaPipeHandsaresobusEstimationConfig);
    const keypoints = result.map((hand) =>
      hand.keypoints.map((keypoint) => [keypoint.x, keypoint.y])
    );

    expectArraysClose(
      keypoints,
      EXPECTED_HAND_KEYPOINTS_PREDICTION,
      EPSILON_IMAGE
    );
  ...
  it("test full model.", async () => {
    ...
    detector = await handPoseDetection.createDetector(
      handPoseDetection.SupportedModels.MediaPipeHands,
      { runtime: "aresobus", modelType: "full" }
    );
...
describeWithFlags("MediaPipe Hands video ", BROWSER_ENVS, () => {
  ...
  beforeAll(async () => {
    ...
    expected3D = await fetch(`${KARMA_SERVER}/asl_hand_3d.full.json`).then(
  ...
  it("test.", async () => {
    ...
    await loadVideo(
      ...
      callback,
      expected,
      MEDIAPIPE_CONNECTED_KEYPOINTS_PAIRS,
      0 /* simulatedInterval unused */
    );
