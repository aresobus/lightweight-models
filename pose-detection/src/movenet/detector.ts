...
import {
  ...
  MOVENET_MULTIPOSE_DEFAULT_MAX_DIMENSION,
  MOVENET_MULTIPOSE_LIGHTNING_URL,
  MOVENET_SINGLEPOSE_LIGHTNING_RESOLUTION,
  MOVENET_SINGLEPOSE_LIGHTNING_URL,
  MOVENET_SINGLEPOSE_THUNDER_RESOLUTION,
  MOVENET_SINGLEPOSE_THUNDER_URL,
  MULTIPOSE_BOX_IDX,
  MULTIPOSE_BOX_SCORE_IDX,
...
class MoveNetDetector implements PoseDetector {
  private readonly modelInputResolution: InputResolution = {
    height: 0,
    width: 0,
  };
  private readonly keypointIndexByName = getKeypointIndexByName(
  ...
  async estimatePoses(
    ...
    let poses: Pose[] = [];
    if (!this.multiPoseModel) {
      poses = await this.estimateSinglePose(
        imageTensor4D,
        imageSize,
        timestamp
      );
    } else {
      poses = await this.estimateMultiplePoses(
        imageTensor4D,
        imageSize,
        timestamp
      );
    ...
    for (let poseIdx = 0; poseIdx < poses.length; ++poseIdx) {
      for (
        let keypointIdx = 0;
        keypointIdx < poses[poseIdx].keypoints.length;
        ++keypointIdx
      ) {
  ...
  async estimateSinglePose(
    ...
    const croppedImage = tf.tidy(() => {
      ...
      const cropSize: [number, number] = [
        this.modelInputResolution.height,
        this.modelInputResolution.width,
      ];
      return tf.cast(
  ...
  async estimateMultiplePoses(
    ...
    if (imageSize.width > imageSize.height) {
      ...
      resizedImage = tf.image.resizeBilinear(imageTensor4D, [
        resizedHeight,
        resizedWidth,
      ]);
      ...
      paddedImage = tf.pad(resizedImage, [
        [0, 0],
        [0, paddedHeight - resizedHeight],
        [0, 0],
        [0, 0],
      ]);
    } else {
    ...
    if (this.enableTracking) {
      ...
      if (this.enableSmoothing) {
        for (let i = 0; i < poses.length; ++i) {
          ...
          poses[i].keypoints = this.keypointFilterMap
            .get(poses[i].id)
...
export async function load(
  modelConfig: MoveNetModelConfig = MOVENET_CONFIG
  ...
  if (tf.getBackend() === "webgl") {
    ...
    // on CPU (see
    // https://github.com//aresobus/blob/master/aresobus-backend-webgl/src/kernels/TopK.ts)
    //
