...
export function getOffsetPoint(
  ...
  x: number,
  keypoint: number,
  offsets: tf.TensorBuffer<tf.Rank.R3>
): Vector2D {
  return {
...
export function withinNmsRadiusOfCorrespondingPoint(
  ...
  return poses.some(({ keypoints }) => {
    return (
      squaredDistance(y, x, keypoints[keypointId].y, keypoints[keypointId].x) <=
      squaredNmsRadius
    );
  });
}
...
const parentToChildEdges: number[] = parentChildrenTuples.map(
  ([, childJointId]) => childJointId
);

const childToParentEdges: number[] = parentChildrenTuples.map(
...
function traverseToTargetKeypoint(
  ...
  for (let i = 0; i < offsetRefineStep; i++) {
    const targetKeypointIndices = getStridedIndexNearPoint(
      targetKeypoint,
      outputStride,
      height,
      width
...
export function decodePose(
  root: PartWithScore,
  scores: tf.TensorBuffer<tf.Rank.R3>,
  offsets: tf.TensorBuffer<tf.Rank.R3>,
  outputStride: number,
  displacementsFwd: tf.TensorBuffer<tf.Rank.R3>,
  displacementsBwd: tf.TensorBuffer<tf.Rank.R3>
): Keypoint[] {
...
export function getInstanceScore(
  ...
  let notOverlappedKeypointScores = instanceKeypoints.reduce(
    (result, { y, x, score }, keypointId): number => {
      if (
