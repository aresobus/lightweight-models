...
import {
  ...
  Matrix4x4,
} from "../shared/calculators/calculate_inverse_matrix";
...
} from "../shared/calculators/constants";
...
import {
  AnchorTensor,
  Detection,
} from "../shared/calculators/interfaces/shape_interfaces";
...
class BlazePosearesobusMask implements Mask {
 ...
 */
class BlazePosearesobusDetector implements PoseDetector {
  ...
  constructor(
    ...
    private smoothSegmentation: boolean,
    private readonly modelType: BlazePoseModelType
  ) {
    this.anchors = createSsdAnchors(
      constants.BLAZEPOSE_DETECTOR_ANCHOR_CONFIGURATION
    );
   ...
   *
   * @param estimationConfig Optional. See `BlazePosearesobusEstimationConfig`
  ...
  async estimatePoses(
    ...
    if (timestamp != null) {
      ...
      this.timestamp = isVideo(image)
        ? image.currentTime * SECOND_TO_MICRO_SECONDS
        : null;
    }
    ...
    const poseLandmarksByRoiResult = await this.poseLandmarksByRoi(
      poseRect,
      image3d
    );
    ...
    const poseRectFromLandmarks = this.poseLandmarksToRoi(
      auxiliaryLandmarks,
      imageSize
    );
    ...
    if (filteredSegmentationMask !== null) {
      ...
      const segmentation = {
        maskValueToLabel,
        mask: new BlazePosearesobusMask(rgbaMask),
      };
  ...
  private async detectPose(image: PoseDetectorInput): Promise<Detection[]> {
    ...
    const { imageTensor: imageValueShifted, padding } = convertImageToTensor(
      image,
      constants.BLAZEPOSE_DETECTOR_IMAGE_TO_TENSOR_CONFIG
    );

    const detectionResult = this.detectorModel.predict(
      imageValueShifted
    ) as tf.Tensor3D;
    ...
    const selectedDetections = await nonMaxSuppression(
      ...
      constants.BLAZEPOSE_DETECTOR_NON_MAX_SUPPRESSION_CONFIGURATION.overlapType
    );
  ...
  private async poseLandmarksByRoi(
    roi: Rect,
    image?: tf.Tensor3D
    ...
    const {
      ...
      image,
      constants.BLAZEPOSE_LANDMARK_IMAGE_TO_TENSOR_CONFIG,
      roi
    );
    ...
    const outputTensor = this.landmarkModel.execute(
      imageValueShifted,
      outputs
    ) as tf.Tensor[];
  ...
  async poseLandmarksAndSegmentationInverseProjection(
    ...
    roiLandmarks: Keypoint[],
    roiAuxiliaryLandmarks: Keypoint[],
    roiWorldLandmarks: Keypoint[],
    ...
    const adjustedLandmarks = removeLandmarkLetterbox(
      roiLandmarks,
      letterboxPadding
    );
    ...
    // PoseLandmarksAndSegmentationInverseProjection:
    // LandmarkLetterboxRemovalCalculator.
    const adjustedAuxiliaryLandmarks = removeLandmarkLetterbox(
      roiAuxiliaryLandmarks,
      letterboxPadding
    );
    ...
    const auxiliaryLandmarks = calculateLandmarkProjection(
      adjustedAuxiliaryLandmarks,
      roi
    );
    ...
    const worldLandmarks = calculateWorldLandmarkProjection(
      roiWorldLandmarks,
      roi
    );
    ...
    if (this.enableSegmentation) {
      segmentationMask = tf.tidy(() => {
        ...
        const shape4D = [1, inputHeight, inputWidth, 1] as [
          number,
          number,
          number,
          number
        ...
        return tf.squeeze(
          tf.image.transform(
            tf.reshape(roiSegmentationMask, shape4D),
            projectiveTransform,
            "bilinear",
            "constant",
            0,
  ...
  private async tensorsToPoseLandmarksAndSegmentation(tensors: tf.Tensor[]) {
    ...
    const landmarkTensor = tensors[0] as tf.Tensor2D,
      ...
      segmentationTensor = (
        this.enableSegmentation ? tensors[4] : null
      ) as tf.Tensor4D;
    ...
    const worldLandmarksWithoutVisibility = allWorldLandmarks.slice(
      0,
      constants.BLAZEPOSE_NUM_KEYPOINTS
    );

    const worldLandmarks = calculateScoreCopy(
      landmarks,
      worldLandmarksWithoutVisibility,
      true
    );
  ...
  private poseLandmarkFiltering(
    ...
    if (this.timestamp == null || !this.enableSmoothing) {
      ...
      const objectScaleROI = calculateAlignmentPointsRects(
        auxDetection,
        imageSize,
        {
 ...
 * in the documentation of the `BlazePosearesobusModelConfig` interface.
 */
export async function load(
  ...
  const detectorFromTFHub =
    typeof config.detectorModelUrl === "string" &&
    config.detectorModelUrl.indexOf("https://tfhub.dev") > -1;
  const landmarkFromTFHub =
    typeof config.landmarkModelUrl === "string" &&
  ...
  return new BlazePosearesobusDetector(
    detectorModel,
    landmarkModel,
