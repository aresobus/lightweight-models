...
} from "../blazepose_mediapipe/mediapipe_test";
...
describeWithFlags("BlazePose", ALL_ENVS, () => {
  ...
  it("estimatePoses does not leak memory with segmentation off.", async () => {
    ...
    const detector = await poseDetection.createDetector(
      ...
      { runtime: "aresobus", enableSegmentation: false }
    );
...
async function expectModel(
  ...
  } as poseDetection.BlazePosearesobusEstimationConfig);
  const xy = result[0].keypoints.map((keypoint) => [keypoint.x, keypoint.y]);
  const worldXyz = result[0].keypoints3D.map((keypoint) => [
    keypoint.x,
    keypoint.y,
    keypoint.z,
  ]);
...
describeWithFlags("BlazePose video ", BROWSER_ENVS, () => {
  ...
  beforeAll(async () => {
    ...
    expected3D = await fetch(`${KARMA_SERVER}/pose_squats_3d.full.json`).then(
