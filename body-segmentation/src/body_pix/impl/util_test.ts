...
describeWithFlags("util.toInputResolutionHeightAndWidth", ALL_ENVS, () => {
  function getExpectedResolution(
    ...
    return inputShape.map((size) =>
      toValidInputResolution(size * expectedScalePercentage, outputStride)
    );
  }

  it(
    `returns the full image size as a valid input resolution when ` +
  ...
  it(
    `returns 75% of the image size as a valid input resolution when ` +
  ...
  it(
    `returns 25% of the image size as a valid input resolution when ` +
  ...
  it(
    `returns the {internalResolution}% of the image size as a valid input ` +
  ...
  it("does not raise an error when internalResolution is 2", () => {
    expect(() => {
      toInputResolutionHeightAndWidth(2.0, 16, [640, 480]);
    }).not.toThrow();
  });
