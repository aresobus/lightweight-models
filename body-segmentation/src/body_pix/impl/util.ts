...
function toInternalResolutionPercentage(
  ...
  if (typeof internalResolution === "string") {
    ...
    tf.util.assert(
      ...
      () =>
        `string value of inputResolution must be one of ${Object.values(
          INTERNAL_RESOLUTION_STRING_OPTIONS
    ...
    tf.util.assert(
      ...
      () =>
        `inputResolution must be a string or number between ${MIN_INTERNAL_RESOLUTION} and ${MAX_INTERNAL_RESOLUTION}, but ` +
...
export function toInputResolutionHeightAndWidth(
  internalResolution: BodyPixInternalResolution,
  outputStride: BodyPixOutputStride,
  [inputHeight, inputWidth]: [number, number]
): [number, number] {
  ...
  return [
    toValidInputResolution(
      inputHeight * internalResolutionPercentage,
      outputStride
    ),
    toValidInputResolution(
      inputWidth * internalResolutionPercentage,
      outputStride
    ),
  ];
}
...
export function resizeAndPadTo(
  ...
  const resizedAndPadded = tf.tidy(() => {
    ...
    if (flipHorizontal) {
      resized = tf.image.resizeBilinear(tf.reverse(imageTensor, 1), [
        resizeH,
        resizeW,
      ]);
    } else {
...
export function scaleAndCropToInputTensorShape(
  ...
  return tf.tidy(() => {
    let inResizedAndPadded: tf.Tensor3D = tf.image.resizeBilinear(
      tensor,
      [resizedAndPaddedHeight, resizedAndPaddedWidth],
      true
    );
