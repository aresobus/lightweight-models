...
describeWithFlags("qna", NODE_ENVS, () => {
  ...
  beforeEach(() => {
    spyOn(tfconv, "loadGraphModel").and.callFake((modelUrl: string) => {
      ...
      executeSpy = spyOn(model, "execute").and.callFake((x: tf.Tensor) => [
        tf.tensor2d(
          [0, 0, 0, 0, 10, 20, 30, 20, 10, 0, ...Array(374).fill(0)],
          [1, 384]
        ),
        tf.tensor2d(
          [0, 0, 0, 0, 10, 20, 30, 20, 10, 20, ...Array(374).fill(0)],
          [1, 384]
        ),
      ]);
  ...
  it("qna detect method should throw error if question is too long", async () => {
    const qna = await load();
  ...
  it("qna detect method should work for long context", async () => {
    ...
    executeSpy.and.returnValue([
      tf.tensor2d(
        [0, 0, 0, 0, 10, 20, 30, 20, 10, 0, ...Array(384 * 6 - 10).fill(0)],
        [6, 384]
      ),
      tf.tensor2d(
        [0, 0, 0, 0, 10, 20, 30, 20, 10, 20, ...Array(384 * 6 - 10).fill(0)],
        [6, 384]
      ),
    ]);
    const data = await qna.findAnswers("question", context);
