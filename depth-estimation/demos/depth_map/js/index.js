...
function createCanvas() {
  ...
  inputTextureFrameBuffer = createTextureFrameBuffer(
    gl,
    gl.LINEAR,
    videoWidth,
    videoHeight
    ...
    if (numInferences % STATE.DepthCachedFrames === 0) {
      ...
      if (faces.length === 0) {
        ...
        const eyesDistance = Math.sqrt(
          tf.util.distSquared(rightEyePoint, leftEyePoint)
        );
        ...
        const portraitPadded = tf.tidy(() => {
          ...
          sliceStart = [
            ...
            Math.max(0, nosePoint[0] - 192 / 2),
            0,
          ];
          sliceSize = [
            Math.min(rescaledEyeHeight - sliceStart[0], 256),
            Math.min(rescaledEyeWidth - sliceStart[1], 192),
            3,
          ];
          ...
          return tf.pad(portraitCrop, [
            [0, 256 - portraitHeight],
            [0, 192 - portraitWidth],
            [0, 0],
          ]);
        });
        ...
        const output = await estimator.estimateDepth(portraitPadded, {
        ...
        tf.tidy(() => {
          ...
          const outputUnpadded = tf.slice(depthMap, 0, [
            portraitHeight,
            portraitWidth,
          ]);
          depthMap.dispose();
          ...
          const outputChannel = tf.expandDims(
            tf.expandDims(outputRescaledEye, 0),
            3
          );

          // Rescale to original input size.
          const outputResized = tf.image.resizeBilinear(outputChannel, [
            videoHeight,
            videoWidth,
          ]);
