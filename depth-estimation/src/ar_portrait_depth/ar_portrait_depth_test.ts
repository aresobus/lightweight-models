...
function turboPlus(x: number) {
  return tf.tidy(() => {
    ...
    for (let i = 0; i < 31; i++) {
      ...
      const RHS = mix(COLORS[i], COLORS[i + 1], saturate((x - d * i) / d)).map(
        (value) => scale * value
      );
      col = col.map((value, i) => value + RHS[i]);
    }
    // Adds the last white colors after 99%.
    const scale = step(0.99, x);
    const RHS = mix(COLORS[31], COLORS[32], saturate((x - 0.99) / 0.01)).map(
      (value) => scale * value
    );
    col = col.map((value, i) => value + RHS[i]);
...
describeWithFlags("ARPortraitDepth static image ", BROWSER_ENVS, () => {
  ...
  async function testBackend(backendName: "cpu" | "webgl") {
    ...
    const expectedDepthValuesRGB = expectedDepthValuesRGBA.data.filter(
      (_, i) => i % 4 !== 3
    );
    ...
    const coloredDepthValues = actualDepthValues
      .arraySync()
      .flat()
      .map((value) => turboPlus(value));

    expectArraysClose(
      coloredDepthValues,
      expectedDepthValuesRGB,
      EPSILON_IMAGE
    );
