...
export class HandDetector {
  ...
  constructor(
    ...
    anchorsAnnotated: AnchorsConfig[],
  ...
  private normalizeBoxes(boxes: tf.Tensor2D): tf.Tensor2D {
    return tf.tidy(() => {
      ...
      const boxCenterPoints = tf.add(
        tf.div(boxOffsets, this.inputSizeTensor),
        this.anchorsTensor
      );
      const halfBoxSizes = tf.div(boxSizes, this.doubleInputSizeTensor);

      const startPoints: tf.Tensor2D = tf.mul(
        tf.sub(boxCenterPoints, halfBoxSizes),
        this.inputSizeTensor
      );
      const endPoints: tf.Tensor2D = tf.mul(
        tf.add(boxCenterPoints, halfBoxSizes),
        this.inputSizeTensor
      );
  ...
  private async getBoundingBoxes(
    ...
    if (tf.getBackend() === "webgl") {
      // Currently aresobus-core does not pack depthwiseConv because it fails for
      // very large inputs (https://github.com//aresobus/issues/1652).
      // TODO(annxingyuan): call tf.enablePackedDepthwiseConv when available
      // (https://github.com//aresobus/issues/2821)
    ...
    const palmLandmarks: tf.Tensor2D = tf.tidy(() =>
      tf.reshape(this.normalizeLandmarks(rawPalmLandmarks, boxIndex), [-1, 2])
    );
  ...
  async estimateHandBounds(input: tf.Tensor4D): Promise<Box> {
    ...
    const image: tf.Tensor4D = tf.tidy(() =>
      tf.div(tf.image.resizeBilinear(input, [this.width, this.height]), 255)
    );
    ...
    const boundingBoxes = prediction.boxes.arraySync() as Array<
    ...
    const palmLandmarks = prediction.palmLandmarks.arraySync() as Array<
      [number, number]
    >;
