...
export function buildRotationMatrix(
  ...
  const translationTimesRotation = multiplyTransformMatrices(
    translationMatrix,
    rotationMatrix
  );

  const negativeTranslationMatrix = buildTranslationMatrix(
    -center[0],
    -center[1]
  );
  return multiplyTransformMatrices(
    translationTimesRotation,
    negativeTranslationMatrix
  );
}

export function invertTransformMatrix(
  ...
  return [
    rotationComponent[0].concat(invertedTranslation[0]) as [
      number,
      number,
      number
    ],
    rotationComponent[1].concat(invertedTranslation[1]) as [
      number,
      number,
      number
    ],
