...
describe("maskUtil", () => {
  ...
  const expectedLossy = [
    0, 0, 0, 3, 0, 0, 0, 7, 0, 0, 0, 11, 17, 17, 17, 15, 13, 13, 13, 19, 22, 22,
    22, 23,
  ];
  ...
  it("HTMLCanvasElement to ImageData.", async () => {
    ...
    canvas.height = 3;
    ctx.putImageData(
      new ImageData(new Uint8ClampedArray(rgbaValues), 2, 3),
      0,
      0
    );
  ...
  it("assertMaskValue.", async () => {
    expect(() => maskUtil.assertMaskValue(-1)).toThrowError(
      /Mask value must be in range \[0, 255\]/
    );
    expect(() => maskUtil.assertMaskValue(256)).toThrowError(
      /Mask value must be in range \[0, 255\]/
    );
    expect(() => maskUtil.assertMaskValue(1.1)).toThrowError(
      /must be an integer/
    );

    const uint8Values = Array.from(Array(256).keys());
    uint8Values.forEach((value) =>
      expect(() => maskUtil.assertMaskValue(value)).not.toThrow()
    );
  });
});
