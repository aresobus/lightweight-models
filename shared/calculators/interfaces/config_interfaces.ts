...
export interface VelocityFilterConfig {
  ...
  velocityScale?: number; // Scale to apply to the velocity calculated over the given
  // window. With higher velocity `low pass filter` weights new new
  // values higher. Lower value adds to lag and to stability.
  minAllowedObjectScale?: number; // If calculated object scale is less than given value smoothing
  // will be disabled and landmarks will be returned as is.
  disableValueScaling?: boolean; // Disable value scaling based on object size and use `1.0`
...
export interface OneEuroFilterConfig {
  ...
  minCutOff?: number; // Minimum cutoff frequency. Start by tuning this parameter while
  ...
  // lag.
  derivateCutOff?: number; // Cutoff frequency for derivate. It is set to 1Hz in the
  // original algorithm, but can be turned to further smooth the
  // speed (i.e. derivate) on the object.
  thresholdCutOff?: number; // The outlier threshold offset, will lead to a generally more
...
export interface AnchorConfig {
  numLayers: number; // Number of output feature maps to generate the anchors on.
  ...
  featureMapWidth: number[]; // Sizes of output feature maps to create anchors. Either
  // featureMap size or stride should be provided.
  featureMapHeight: number[];
  strides: number[]; // Strides of each output feature maps.
  aspectRatios: number[]; // List of different aspect ratio to generate anchors.
  fixedAnchorSize?: boolean; // Whether use fixed width and height (e.g. both 1.0) for each
  // anchor. This option can be used when the predicted anchor
  // width and height are in pixels.
  reduceBoxesInLowestLayer?: boolean; // A boolean to indicate whether the fixed 3 boxes per location
  // is used in the lowest layer.
  interpolatedScaleAspectRatio?: number; // An additional anchor is added with this aspect ratio and a
...
export interface TensorsToDetectionsConfig {
  numClasses: number; // The number of output classes predicted by the detection model.
  numBoxes: number; // The number of output boxes predicted by the detection model.
  ...
  // y-direction).
  minScoreThresh?: number; // Score threshold for perserving decoded detections.
}
...
export interface LandmarksRefinementConfig {
  indexesMapping: number[]; // Maps indexes of the given set of landmarks to indexes of the
