...
describe("ImageToTensorCalculator", () => {
  ...
  async function runTest(
    ...
    const actualResult = tf.tidy(() =>
      tf.add(tf.mul(imageTensor, scale), offset)
    );
    tf.dispose(imageTensor);

    const expectedResultRGBA = await loadImage(
      `shared/${expectedFileName}`,
      config.outputTensorSize.width,
      config.outputTensorSize.height
    ).then((image) => toImageDataLossy(image));
    const expectedResultRGB = expectedResultRGBA.data.filter(
      (_, index) => index % 4 !== 3
    );

    expectArraysClose(
      new Uint8ClampedArray(actualResult.dataSync()),
      expectedResultRGB,
      EPS
    );
  }
